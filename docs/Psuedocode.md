...
## Display Function
```python
def display(contents, window, cursorline, cursorcol):
	print("\x1b[H")
	lnlen = (int)math.log10((double)contents.count) + 1
	for line in range(window, min(window + console_window_height, len(contents))):
		s = new(contents[line])
		print("\x1b[7m" + line.ToString().PadRight(lnlen) + "\x1b[27m " + s + "\x1b[0K")
	print("\x1b[0m\x1b[" + (cursorline+1).ToString() + ";" + (cursorcol+lnlen+2).ToString() + "H")
